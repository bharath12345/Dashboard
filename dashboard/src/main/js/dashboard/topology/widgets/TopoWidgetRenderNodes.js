define(["dojo/_base/declare", "dojo/i18n",
    ...
    function (declare, i18n, TitlePane, GridContainer, i18nString, Logger, TOPOLOGYCONSTANTS, TopologyUtility) {
        ...
        dashboard.classnames.TopoWidgetRenderNodes = "dashboard.topology.widgets.TopoWidgetRenderNodes";

        var TopoWidgetRenderNodes = declare(dashboard.classnames.TopoWidgetRenderNodes, null, {
            ...
            createEndpoint:function (endPointsArray, endPointType, width, height) {
                ...
                for (var i = 0; i < endPointsArray.length; i++) {
                    var divCol = dojo.byId(endPointType + TopoWidgetRenderNodes.ROW_SUFFIX);
                    var node = dojo.create("div");
                    node.id = endPointType + "_" + endPointsArray[i] + TopoWidgetRenderNodes.ENDPOINT_SUFFIX;
                ...
                try {
                    ...
                    for (var i = 0; i < names.length; i++) {
                        ...
                        var titlePane = new TitlePane({
                            splitter:false,
                            style:styleString,
                            content:"<div id='" + names[i] + TopoWidgetRenderNodes.COLUMN_SUFFIX + "' style='width: 100%; height: 100%;'></div>",
                            title:names[i],
                            toggleable:false
                        });
                    ...
                    dashboard.topology.views.TopologyView.TitlePane.addChild(gridContainer);
                ...
                try {
                    ...
                    for (var i = 0; i < layers.length; i++) {
                        var innerPane = new TitlePane({
                            splitter:false,
                            style:styleString,
                            content:"<div id='" + layers[i] + TopoWidgetRenderNodes.ROW_SUFFIX + "' style='" + styleString + "'></div>",
                            title:layers[i],
                            toggleable:false
                        });
                    ...
                    var gridContainer = new GridContainer({nbZones:1, isAutoOrganized:true}, dojo.byId(name + TopoWidgetRenderNodes.COLUMN_SUFFIX));
                    gridContainer.disableDnd();
            ...
            create:function (data, input) {
                ...
                var innerPane = dojo.query(".dijitTitlePaneContentOuter", dashboard.topology.views.TopologyView.CP.domNode);
                for (var i = 0; i < innerPane.length; i++) {
                    innerPane[i].style.border = 0;
                }

                var gridText = dojo.query(".dijitTitlePaneTextNode", dashboard.topology.views.TopologyView.CP.domNode);
        ...
        TopoWidgetRenderNodes.LOG = Logger.addTimer(new Logger(dashboard.classnames.TopoWidgetRenderNodes));

        TopoWidgetRenderNodes.TYPE = {};
        TopoWidgetRenderNodes.TYPE.WEBSERVER = "WebServers";
        TopoWidgetRenderNodes.TYPE.APPSERVER = "AppServers";
        TopoWidgetRenderNodes.TYPE.DATABASES = "Databases";
        TopoWidgetRenderNodes.TYPE.MESSAGEQ = "MessageQueues";
        TopoWidgetRenderNodes.TYPE.TCPENDPOINTS = "TcpEndpoints";

        TopoWidgetRenderNodes.COLUMN_SUFFIX = "_col";
        TopoWidgetRenderNodes.ROW_SUFFIX = "_row";
        TopoWidgetRenderNodes.ENDPOINT_SUFFIX = "_endpoint";

        return TopoWidgetRenderNodes;
    });